...
def convolution(tensor_in, filters, reuse=False, name=None):
...
class Classifier:
    ...
    def __call__(self, input):
        ...
        with tf.variable_scope(self.name):
            ...
            conv_1 = convolution(input, 64, reuse=self.reuse, name="conv_1")
            conv_2 = convolution(conv_1, 64, reuse=self.reuse, name="conv_2")
            pool_1 = pool(conv_2, reuse=self.reuse, name="pool_1")
            # print("pool_1", pool_1)

            conv_3 = convolution(pool_1, 128, reuse=self.reuse, name="conv_3")
            conv_4 = convolution(conv_3, 128, reuse=self.reuse, name="conv_4")
            pool_2 = pool(conv_4, reuse=self.reuse, name="pool_2")
            # print("pool_2", pool_2)

            conv_5 = convolution(pool_2, 256, reuse=self.reuse, name="conv_5")
            conv_6 = convolution(conv_5, 256, reuse=self.reuse, name="conv_6")
            conv_7 = convolution(conv_6, 256, reuse=self.reuse, name="conv_7")
            pool_3 = pool(conv_7, reuse=self.reuse, name="pool_3")
            # print("pool_3", pool_3)

            conv_8 = convolution(pool_3, 512, reuse=self.reuse, name="conv_8")
            conv_9 = convolution(conv_8, 512, reuse=self.reuse, name="conv_9")
            conv_10 = convolution(conv_9, 512, reuse=self.reuse, name="conv_10")
            pool_4 = pool(conv_10, reuse=self.reuse, name="pool_4")
            # print("pool_4", pool_4)

            conv_11 = convolution(pool_4, 512, reuse=self.reuse, name="conv_11")
            conv_12 = convolution(conv_11, 512, reuse=self.reuse, name="conv_12")
            conv_13 = convolution(conv_12, 512, reuse=self.reuse, name="conv_13")
            ...
            dense_3 = tf.layers.dense(dense_2, 100, activation=tf.nn.relu, reuse=self.reuse,
